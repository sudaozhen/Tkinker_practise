...
def usr_sign_up():
    def sign_to_Mofan_Python():
        ...
        if np != npf:
            ...
            tk.messagebox.showerror('Error','The user name has already signed up!')
        else:
    ...
    tk.Label(window_sign_up, text='Password:').place(x=10, y=10)
    entry_usr_pwd = tk.Entry(window_sign_up, textvariable=new_pwd, show='*')
    entry_usr_pwd.place(x=150, y=50)

    new_pwd_confirm = tk.StringVar()
    tk.Label(window_sign_up, text='Confirm password:').place(x=10, y=10)
